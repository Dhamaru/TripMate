...
const CASES: Case[] = [
    { name: "list trips (empty account)", message: "What are my current trips?", expectToolOneOf: ["list_trips"] },
    { name: "weather lookup", message: "What's the weather like in Tokyo right now?", expectToolOneOf: ["get_weather"] },
    { name: "currency conversion", message: "Convert 100 USD to INR", expectToolOneOf: ["convert_currency"] },
    { name: "translation", message: "How do you say 'thank you' in Japanese?", expectToolOneOf: ["translate_text"] },
    { name: "places search", message: "Find me some good restaurants in Paris", expectToolOneOf: ["search_places"] },
    { name: "emergency info", message: "What's the emergency number in Italy?", expectToolOneOf: ["get_emergency_info"] },
    { name: "travel hacks", message: "Give me budget travel tips for Bali", expectToolOneOf: ["get_travel_hacks"] },
    { name: "packing list generation", message: "What should I pack for a week in Iceland in winter?", expectToolOneOf: ["get_weather", "generate_packing_list"] },
    { name: "budget breakdown", message: "I have a $2000 budget for a standard trip to Rome from New York, how should I split it?", expectToolOneOf: ["get_budget_breakdown"] },
    { name: "preferences stated", message: "I'm vegan and I live in Berlin, please remember that", expectToolOneOf: ["update_user_preferences"] },
    { name: "preferences read", message: "What do you know about my travel preferences?", expectToolOneOf: ["get_user_preferences"] },
    { name: "general question, no tool needed", message: "What's a good rule of thumb for tipping in the US?" },
    ...
    for (const c of CASES) {
        ...
        try {
            ...
            if (toolOk && textOk && hasReply) {
                ...
                console.log(`FAIL  ${c.name} (${elapsed}ms) — expected tool one of [${c.expectToolOneOf?.join(", ")}], got [${toolsUsed.join(", ")}]. Reply: "${text.slice(0, 120)}"`);
                fail++;
            }
        } catch (err: any) {
