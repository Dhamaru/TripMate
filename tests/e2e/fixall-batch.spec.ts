 ...
 *
 * Regression suite for the 6 bugs fixed in commit f61bcf0.
 ...
 *
 * Live evidence from investigation run (2026-09-03):
 *   Bug 2: [] and {cuisine:[BBQ,Steakhouse],dietary:[Vegetarian]} both persist correctly.
 *   Bug 4: 500-injected hotel search -> error banner visible; retry -> backend hit -> error cleared.
 *   Bug 5: Banner changed Cafe1947 -> LazyDogLounge on second click; cleared by show-all.
 *   Bug 6: custom-marker count stable at 9 across 2 different View-on-Map clicks (no accumulation).
 */
...
test("[Bug1] Ungeocodeable activity added by Atlas still persists (no crash), row renders gracefully", async () => {
  ...
  try {
    ...
    try {
      ...
      expect(vip.length, "View-in-Places button present for restaurant").toBeGreaterThanOrEqual(1);
    } finally {
      await page.close();
    }
  } finally {
...
test("[Bug5] Clicking View-in-Places on second restaurant updates banner to that restaurant", async () => {
  ...
  try {
    ...
    await page.waitForSelector("button[title]", { timeout: 15000 }).catch(() => {});
    await sleep(3000);
...
// ── Bug 6: View-on-Map ad-hoc marker does not accumulate ──────────────────
...
test("[Bug6] Clicking View-on-Map on a second Places result removes the first ad-hoc marker", async () => {
  ...
  try {
    ...
    const ptab = page.getByRole("tab").filter({ hasText: /places/i });
    await expect(ptab, "places tab visible").toBeVisible({ timeout: 8000 });
    await ptab.click({ force: true });
    await sleep(2000);
