...
export function AtlasDayButton({ dayIndex, destination, activities, tripId }: Props) {
    ...
    return (
        <button
            className="w-[calc(100%-2rem)] mx-4 mb-4 py-2 px-4 rounded-lg border border-dashed border-border text-sm text-muted-foreground hover:border-[#163F73]/50 hover:text-[#163F73] transition-colors flex items-center justify-center gap-2"
            onClick={handleClick}
