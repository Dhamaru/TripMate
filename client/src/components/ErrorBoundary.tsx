...
export default class ErrorBoundary extends React.Component<React.PropsWithChildren<{}>, ErrorBoundaryState> {
  ...
  render() {
    if (this.state.hasError) {
      return (
        <div className="min-h-screen bg-muted text-white flex items-center justify-center p-6">
          <div className="max-w-md text-center">
            <h1 className="text-2xl font-semibold mb-2">Something went wrong</h1>
            <p className="text-muted-foreground mb-4">
              {this.state.error?.message || "Please refresh the page."}
            </p>
            <details className="text-xs text-muted-foreground/50 mb-4 text-left bg-black/20 p-2 rounded overflow-auto max-h-32">
