...
const inputCls = "w-full bg-muted border border-border rounded-xl px-4 py-3 text-foreground placeholder:text-muted-foreground outline-none focus:border-[#163F73] focus:ring-1 focus:ring-[#163F73]/40 transition-colors text-sm"
const primaryBtn = "py-3 px-6 rounded-xl font-semibold text-sm bg-[#163F73] hover:bg-[#0F2C52] text-white transition-all hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100"
...
const chipActive = `${chipBase} bg-[#163F73] text-white border-[#163F73]`
const chipInactive = `${chipBase} bg-muted text-muted-foreground border-border hover:border-[#163F73]/50`
        ...
        <div className="min-h-screen flex items-center justify-center p-4 bg-background">
            <div className="w-full max-w-xl bg-card rounded-2xl shadow-2xl p-6 md:p-10 border border-border">
                ...
                {step <= 4 && (
                    <div className="mb-8">
                        <div className="flex items-center justify-between relative">
                            ...
                            <div
                                className="absolute left-0 top-4 h-0.5 bg-[#163F73] transition-all duration-500 -z-0"
                                ...
                                return (
                                    <div key={s.n} className="flex flex-col items-center gap-1.5 z-10">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all duration-300 ${
                                            done ? 'bg-[#163F73] border-[#163F73] text-white' :
                                            active ? 'bg-card border-[#163F73] text-[#163F73]' :
                                            'bg-card border-border text-muted-foreground'
                                        }`}>
                                            {done ? '✓' : s.n}
                                        </div>
                                        <span className={`text-[10px] font-medium hidden sm:block ${active ? 'text-[#163F73]' : done ? 'text-foreground' : 'text-muted-foreground'}`}>
                                            {s.label}
                                        </span>
                                    </div>
                                )
                ...
                {step === 2 && (
                    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        {form.destination && (
                            <div className="inline-flex items-center gap-1.5 text-xs font-semibold bg-[#163F73]/10 text-[#163F73] px-3 py-1 rounded-full border border-[#163F73]/20">
                                📍 {form.destination}
                            </div>
                        )}
                        ...
                        <div className="space-y-4">
                            <label className="block text-sm font-medium text-muted-foreground">
                                ...
                                <div className="flex gap-2 mt-2">
                                    ...
                                    <select className="w-24 bg-muted border border-border rounded-xl px-3 py-3 text-foreground outline-none focus:border-[#163F73] transition-colors text-sm" value={form.currency} onChange={e => update('currency', e.target.value)} aria-label="Currency">
                ...
                {step === 5 && (
                    <div className="space-y-6 text-center animate-in fade-in zoom-in-95 duration-500 py-6">
                        <div className="relative w-20 h-20 mx-auto">
                            ...
                            <div className="absolute inset-0 w-20 h-20 rounded-full border-4 border-[#163F73] border-t-transparent animate-spin" aria-hidden="true" />
                        ...
                        <div className="text-left bg-muted rounded-xl p-4 space-y-2 border border-border" aria-live="polite" aria-label="Generation progress">
                            {liveSteps.length === 0 && (
                                <div className="flex items-center gap-2 text-sm text-muted-foreground animate-pulse py-1">
                                    <span className="w-4 h-4 rounded-full border-2 border-[#163F73] border-t-transparent animate-spin shrink-0" />
                                    Starting up...
                                </div>
                            )}
                            {liveSteps.map((s, i) => {
