...
export function SortablePackingItem({
  item,
  handleToggle,
  handleDelete,
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className={`w-6 h-6 rounded-full border-2 mr-4 flex items-center justify-center transition-colors ${item.packed ? "bg-[#1D4E89] border-[#1D4E89]" : "border-border group-hover:border-[#163F73]/60"}`}
        >
        ...
        <div className="flex-1 flex flex-wrap items-center gap-1.5 select-none">
          <span
            className={`font-medium ${item.packed ? "text-muted-foreground line-through" : "text-foreground"}`}
          >
            {item.name}
          </span>
          {item.is_mandatory && (
            <span className="stamp text-[9px] px-1.5 py-0 border-[var(--stamp-red)] text-[var(--stamp-red)]">
              Essential
            </span>
          )}
          {item.weatherDriven && (
            <span className="stamp text-[9px] px-1.5 py-0 border-[var(--transit-green)] text-[var(--transit-green)]">
              Weather
            </span>
          )}
          {item.activityDriven && (
            <span className="stamp text-[9px] px-1.5 py-0 border-[var(--customs-blue)] text-[var(--customs-blue)]">
              Activity
            </span>
          )}
          {item.category && (
            <span className="text-[10px] font-mono-data text-muted-foreground bg-muted px-2 py-0.5 rounded-[4px] border border-border">
              {item.category}
            </span>
          )}
        </div>
