        ...
        >
            {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-full gap-2 text-center px-6">
                    <span className="w-12 h-12 rounded-full bg-[var(--explorer-blue)]/10 flex items-center justify-center text-2xl mb-1" aria-hidden="true">🌍</span>
                    <p className="text-foreground text-sm font-medium">Ask Atlas anything about your trip</p>
                    <p className="text-muted-foreground text-xs max-w-[240px]">Weather, budgets, packing, places to go — or just tell it what you're planning.</p>
                </div>
            )}
            {messages.map((msg) => (
                ...
                <div key={msg.id} className={`flex gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {msg.role === 'assistant' && (
                        <div className="w-7 h-7 rounded-full bg-[var(--explorer-blue)] flex items-center justify-center flex-shrink-0 mt-0.5 text-[10px] font-bold text-white">A</div>
                    )}
                    <div
                        className={
                            msg.role === 'user'
