        ...
        >
            ...
            {messages.map((msg) => (
                <div key={msg.id} className={`flex gap-2 ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                    {msg.role === 'assistant' && (
                        <div className="w-7 h-7 rounded-full bg-[#1E3A8A] flex items-center justify-center flex-shrink-0 mt-0.5 text-[10px] font-bold text-white">A</div>
                    )}
                    <div
                        className={
                            msg.role === 'user'
                                ? "max-w-[78%] px-4 py-2.5 rounded-2xl rounded-br-sm bg-[#F59E0B] text-white text-sm leading-relaxed shadow-sm"
