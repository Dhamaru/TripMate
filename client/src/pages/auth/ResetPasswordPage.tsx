...
export default function ResetPasswordPage() {
  ...
  async function onSubmit(data: ResetPasswordForm) {
    ...
    setIsLoading(true);
    try {
      // Raw fetch() previously skipped the CSRF header apiRequest
      // attaches — silently 403'd in production for any visitor who
      // still had a stale auth cookie present.
      ...
      <Card className="bg-white/10 backdrop-blur-xl border-white/15 radius-card p-1 shadow-2xl overflow-hidden">
        <CardContent className="p-8 text-center pt-12 pb-12">
          ...
          <Button
            ...
            variant="outline"
            className="w-full h-14 bg-white/10 border-white/20 rounded-xl text-white hover:bg-white/10 font-bold"
          >
            Back to Sign In
          </Button>
        </CardContent>
    ...
    <Card className="bg-white/10 backdrop-blur-xl border-white/15 radius-card p-1 shadow-2xl overflow-hidden">
      <CardContent className="p-8">
        <div className="flex flex-col items-center text-center mt-2 mb-8">
          ...
          <p className="text-white/60 font-medium">Create a strong new password for your account</p>
        </div>
        ...
        <div className="mt-8 text-center">
          <Link href="/signin">
            <Button
              variant="ghost"
              size="sm"
              className="text-white/60 hover:text-white hover:bg-transparent font-semibold"
            >
              Back to Sign In
            </Button>
          </Link>
