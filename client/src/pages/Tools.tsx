...
const TOOLS = [
  ...
  },
  {
    ...
    description: "7-day forecasts for any destination",
    icon: CloudSun,
    href: "/app/weather",
  ...
  },
  {
    ...
    description: "Local emergency numbers & services",
    icon: Siren,
    href: "/app/emergency",
    tint: "icon-tint-red",
    num: "06",
        ...
        {TOOLS.map((tool) => (
          <Link key={tool.title} href={tool.href}>
            <div className="bg-[hsl(var(--card))] rounded-xl border border-[hsl(var(--border))] px-5 py-4 flex items-center gap-5 card-hover-glow cursor-pointer group transition-all">
              {/* Number */}
              <span className="stat-display text-2xl text-[hsl(var(--muted-foreground))]/30 group-hover:text-[rgb(var(--amber-rgb)/50%)] transition-colors w-8 flex-shrink-0 leading-none select-none">
                {tool.num}
              </span>

              {/* Icon */}
              <div
                className={`w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 ${tool.tint} group-hover:scale-110 transition-transform duration-200`}
              >
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-[14px] font-semibold text-[hsl(var(--foreground))] font-sans-clean">
                  {tool.title}
                </h3>
                <p className="text-[hsl(var(--muted-foreground))] text-[12px] mt-0.5 font-sans-clean">
                  {tool.description}
                </p>
              </div>
