...
export default function Journal() {
  const {
    ...
    isAuthenticated,
  } = useAuth() as { user: User | undefined; isLoading: boolean; isAuthenticated: boolean };
  ...
  const handleAiEnhance = async () => {
    ...
    try {
      ...
      setEntryForm((prev) => ({
        ...
        title:
          prev.title ||
          (data.suggestedLabels?.[0] ? `My ${data.suggestedLabels[0]} Trip` : prev.title),
      }));
      toast({ title: "Entry Enhanced!", description: `Tone: ${data.sentiment}` });
    } catch (err) {
      toast({
        title: "AI Error",
        description: err instanceof Error ? err.message : "Failed to enhance entry.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleEdit = async (entry: JournalEntry) => {
    ...
    try {
      ...
      /* keep the light-list fallback already set above */
    }
  };
  ...
  if (authLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        <div className="animate-spin rounded-full h-10 w-10 border-b-2 border-[var(--amber)]" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
          <DialogTrigger asChild>
            <Button
              className="bg-[var(--amber)] hover:bg-[var(--airbnb-primary-active)] text-white rounded-xl gap-2"
            ...
            <form onSubmit={handleSubmit} className="space-y-4" data-testid="journal-entry-form">
                ...
                <div className="flex justify-between items-center mb-1.5">
                  ...
                  <div className="flex gap-2">
                    <Button
                      ...
                      className="text-[var(--amber)] hover:text-[var(--amber)] hover:bg-[var(--amber-dim)] h-7 text-xs gap-1"
                    >
              ...
              <div>
                ...
                <Input
                  ...
                  className="bg-muted/50 border text-foreground file:bg-[var(--amber)] file:text-white file:border-0 file:rounded-md file:px-2 file:py-1 file:mr-2 file:hover:bg-[var(--airbnb-primary-active)] cursor-pointer"
                />
                {photos && photos.length > 0 && (
                  <div className="mt-2 p-3 bg-[var(--amber-dim)] border border-[var(--amber-hover-border)] rounded-xl">
                    <p className="text-sm text-[var(--amber)] font-semibold mb-1">
                      {photos.length} file{photos.length > 1 ? "s" : ""} selected
                    </p>
                    <ul className="text-xs text-muted-foreground space-y-0.5">
                      {Array.from(photos).map((file, i) => (
                        <li key={i} className="flex items-center gap-2">
                          <span className="text-[var(--emerald-horizon)]">✓</span>
              ...
              <div className="flex gap-2 pt-2">
                ...
                <Button
                  ...
                  className="flex-1 bg-[var(--amber)] hover:bg-[var(--airbnb-primary-active)] text-white"
      ...
      {isLoading ? (
              ...
              <div key={date} className="relative pl-6 md:pl-10">
                ...
                <div className="grid grid-cols-1 gap-4">
                  {entries.map((entry) => (
                    <div
                      key={entry.id}
                      className="bg-card rounded-3xl border border hover:border-[var(--amber-hover-border)] hover:shadow-md transition-all cursor-pointer group overflow-hidden"
                      onClick={() => navigate(`/app/journal/${entry.id}`)}
                    >
                      <div className="flex flex-row h-32 sm:h-40">
                        <div className="w-1/3 min-w-[100px] border-r border-[hsl(var(--border))] flex-shrink-0">
                        ...
                        <div className="w-2/3 p-3 sm:p-4 flex flex-col justify-between overflow-hidden">
                          <div>
                            <div className="flex justify-between items-start mb-1 gap-2">
                              ...
                              {entry.location && (
                                <span className="text-[10px] text-[var(--amber)] bg-[var(--amber-dim)] px-1.5 py-0.5 rounded-full flex-shrink-0 flex items-center border border-[var(--amber-hover-border)]">
        ...
        <div
          className="bg-card rounded-3xl border border p-16 text-center"
          data-testid="no-journal-entries"
        >
          <div className="w-16 h-16 bg-[var(--amber-dim)] rounded-3xl flex items-center justify-center mx-auto mb-5">
            <Book className="h-8 w-8 icon-tint-amber" />
          </div>
          <h3 className="text-xl font-bold text-foreground mb-2">Start Your Travel Journal</h3>
          <p className="text-muted-foreground text-sm mb-6 max-w-sm mx-auto">
            Document your adventures, capture memories, and create a beautiful record of your
            travels.
          </p>
          <Button
            onClick={() => {
