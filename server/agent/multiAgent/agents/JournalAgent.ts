...
export class JournalAgent extends BaseAgent<JournalResult> {
  ...
  ): Promise<AgentResult<JournalResult>> {
    ...
    try {
      ...
      const messages = [
        { role: "system" as const, content: systemPrompt },
        {
          role: "user" as const,
          content:
            "Process the provided journal entry (contextualize, enhance, or recap depending on inputs).",
        },
      ];
      ...
      const { text, tokensUsed, toolCallCount } = await this.llmCall(input, messages, tools);
      ...
      return this.createResult(false, null, error.message, 0, 0, Date.now() - startTime, 0);
    }
  }
}
