...
// OpenRouter added as a fourth, independent free-tier pool — Groq's 12k
// TPM/60s ceiling and NVIDIA's shared capacity are the two things that
// have caused the majority of real production failures this session, and
// OpenRouter's free models draw from a completely separate quota, so
// putting it first gives requests a real chance to avoid both. Verified
// live before wiring in: minimax/minimax-m2.7:free correctly calls tools
// for real requests, doesn't hallucinate a tool call on a plain greeting,
// AND (unlike several other free candidates tried, including a 120B
// NVIDIA model on the same OpenRouter pool) correctly follows an explicit
...
async function summarizeIfNeeded(
    ...
    messages: [
      {
        role: "system",
        content:
          "Summarize this conversation in 3 sentences, preserving key decisions, trip details, and data.",
      },
      {
        role: "user",
        content: toSummarize
  ...
  return [...(systemMsg ? [systemMsg] : []), summaryMessage, ...recentMessages];
}
...
export async function runAgentLoop(
  ...
  let pendingConfirmation: { id: string; toolName: string; summary: string } | undefined =
    undefined;
  const mutations: Mutation[] = [];
  const groundingSignals = { mentions: 0, verified: 0 };
  let lastIteration = 0;
  ...
  const allProvidersExhausted = MODELS.every(
    (_, i) => isProviderOpen(i) || !hasTokenBudget(i, preflightEstimate),
  );
  if (allProvidersExhausted) {
    console.warn(
      "[Atlas:Loop] All providers circuit-open or over token budget — failing fast instead of a doomed attempt.",
    );
    return {
      message:
        "Atlas is handling a lot of requests right now and needs a moment to catch up — please try again in about a minute.",
      toolsUsed: [],
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "[Atlas:Loop] Context summarization failed, proceeding with full history:",
        summarizeErr,
      );
    }
    for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
      ...
      const estimatedRequestTokens =
        estimateTokens(JSON.stringify(messages)) + toolsTokenEstimate + 1024;
      while (
        currentModelIndex < MODELS.length - 1 &&
        (isProviderOpen(currentModelIndex) ||
          !hasTokenBudget(currentModelIndex, estimatedRequestTokens))
      ) {
        const why = isProviderOpen(currentModelIndex)
          ? "circuit open (repeated recent failures)"
          : "insufficient token budget in current window";
      ...
      try {
        ...
        if (shouldFallback) {
          ...
          if (currentModelIndex < MODELS.length - 1) {
            console.warn(
              `[Atlas:Loop] ${reason} hit for ${MODELS[currentModelIndex]} (${apiErr?.message}). Falling back to ${MODELS[currentModelIndex + 1]}`,
            );
            currentModelIndex++;
      ...
      try {
        ...
        if (shouldFallback) {
          ...
          if (currentModelIndex < MODELS.length - 1) {
            console.warn(
              `[Atlas:Loop] ${reason} hit mid-stream for ${MODELS[currentModelIndex]} (${streamErr?.message}). Falling back to ${MODELS[currentModelIndex + 1]}`,
            );
            currentModelIndex++;
            iteration--;
            continue;
      ...
      if (message.tool_calls && message.tool_calls.length > 0) {
        ...
        const toolMessages = (await Promise.all(toolExecutionPromises)).filter(
          (m) => m !== null,
  ...
  if (!finalText && totalToolCalls > 0) {
    finalText =
      "I've gathered some information but haven't formed a final response. Please try again or be more specific.";
  }
  ...
  const grounding =
    totalToolCalls === 0 ? 1.0 : Math.min(1, groundingSignals.verified / (totalToolCalls || 1));
