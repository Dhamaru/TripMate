...
export async function tripPlannerHandler(args: {
  tripId: string;
  userId: string;
  itinerary: any[];
}): Promise<ToolResult> {
  ...
  try {
    ...
    if (!tripId || !userId || !itinerary) {
      return {
        success: false,
        error: "tripId, userId, and itinerary are required",
    ...
    // Clean the itinerary structure for Mongoose while PRESERVING every
    // activity field the model sends (type, cost, entryFee,
    // duration_minutes, placeName, address, lat/lon, ...). The old
    // explicit allow-list silently dropped all of those, so Atlas-planned
    // activities showed $0 in the budget tab and lost their type routing.
    ...
    return {
      success: false,
      error: err.message || "Trip finalization failed",
      durationMs: Date.now() - start,
    };
  }
}
