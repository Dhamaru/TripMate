...
export async function weatherHandler(args: {
  location: string;
  units?: string;
}): Promise<ToolResult> {
  ...
  try {
    ...
    if (!geo) {
      return {
        success: false,
        error: `Could not geocode location: ${args.location}`,
    ...
    const res = await fetch(url);
    if (!res.ok) {
      // Tool errors get read and paraphrased straight into Atlas's reply
      // (see systemPrompt.ts's "explain and offer an alternative" rule)
      // — a raw "Open-Meteo API error: 429" surfaced to a real user as
      // Atlas saying "rate limit exceeded", exposing an internal
      // provider name/status code. Keep this message user-appropriate.
      const error =
        res.status === 429
          ? "Weather data is temporarily unavailable — the forecast provider is under heavy load right now. Please try again in a moment."
          : "Weather data is temporarily unavailable right now.";
    ...
    const precipChances =
      forecast.length > 0
        ? forecast.map((f: { precipitationChance: number }) => f.precipitationChance)
        : [0];
    ...
    const precipitationChance = Math.round(
      precipChances.reduce((a: number, b: number) => a + b, 0) / (precipChances.length || 1),
    );
