...
export async function tripHandler(
    args: { tripId: string; userId: string; action?: string; dayIndex?: number; updatedActivities?: any[] },
): Promise<ToolResult> {
    ...
    try {
        ...
        if (args.action === 'update_day' && typeof args.dayIndex === 'number' && args.updatedActivities) {
        ...
        return { success: false, error: err.message || 'Trip operation failed', durationMs: Date.now() - start };
    }
}
