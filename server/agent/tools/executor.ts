...
export async function executeTool(
  ...
  if (needsConfirm && needsConfirm(args) && context.userId) {
    ...
    return {
      success: false,
      error:
        "Confirmation required: tell the user exactly what this action will do and that they need to confirm it themselves using the button shown — you cannot confirm it for them.",
...
export async function dispatchTool(
  ...
  try {
    switch (name) {
      ...
      case "translate_text":
        result = await translateHandler(
          args as { text: string; sourceLang?: string; targetLang: string },
        );
        break;
      ...
      case "get_emergency_info":
        result = await emergencyHandler(args as { destination: string; infoType?: string }, {
          aiService: deps.aiService,
        });
        break;
      ...
      case "get_travel_hacks": {
        ...
        try {
          ...
          result = {
            success: false,
            error: err.message || "Travel hacks failed",
            durationMs: Date.now() - hackStart,
          };
        }
        break;
      }
      ...
      case "augment_journal": {
        ...
        try {
          ...
          result = {
            success: false,
            error: err.message || "Journal augmentation failed",
            durationMs: Date.now() - augStart,
          };
        }
        break;
      }
      ...
      case "collaborate_with_agents": {
        ...
        try {
          ...
          result = {
            success: false,
            error: err.message || "Collaboration failed",
            durationMs: Date.now() - collStart,
          };
        }
        break;
