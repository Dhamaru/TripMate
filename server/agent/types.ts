...
export interface Message {
  ...
  tool_call_id?: string;
  name?: string;
  // Present when read back via AtlasMemoryService.getHistory (the DB
  // stores it — see memory.ts's addMessages — but every other producer
  // of a Message in this codebase, e.g. agentLoop.ts building a fresh
  // turn, has no timestamp yet); optional rather than widening every
  // call site that constructs one without it.
  timestamp?: string;
}
