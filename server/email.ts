...
async function createTransporter() {
  ...
  console.log(
    `Configuring SMTP transport: Host=${configData.host}, Port=${configData.port}, Secure=${configData.secure}`,
  );
...
export async function sendEmail(options: nodemailer.SendMailOptions) {
  try {
    ...
    const isEthereal =
      info.messageId &&
      ((transporter as any).transporter?.options as any)?.host === "smtp.ethereal.email";
...
<a href="${tripUrl}" style="display:inline-block;background:#F59E0B;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:bold;margin:16px 0">Open Trip</a>
  ...
  try {
    ...
    const info = await transporter.sendMail({
      from: `"TripMate" <${config.SMTP_FROM_EMAIL || config.SMTP_USER || "noreply@tripmate.app"}>`,
      to: toEmail,
      subject,
      text,
      html,
...
<p style="color:#666;font-size:13px">If you did not request a password reset, ignore this email — your password won't change.</p>
  ...
  try {
    ...
    if (!transporter) throw new Error("Email transporter not initialized");
    const info = await transporter.sendMail({
      from: `"TripMate Support" <${config.SMTP_FROM_EMAIL || config.SMTP_USER || "noreply@tripmate.app"}>`,
      to: email,
      subject,
      text,
      html,
...
// New-login alert — the actual mitigation for "I hold the DB credentials,
// so I could technically sign in as a friend/family member's account":
// nothing can make that technically impossible for whoever holds the
// Mongo connection string, but this means the account owner finds out.
// Fired from signin() only (not signup/guest — those are the account's
// first-ever session, not a new one on top of existing ones) whenever
// the request's User-Agent doesn't match any non-revoked session already
// on file for that user.
export async function sendNewLoginAlertEmail(email: string, ip: string, userAgent: string) {
  const when = new Date().toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" });
  const subject = "New sign-in to your TripMate account";
  const text = `Your TripMate account was just signed into from a device we haven't seen before.\n\nWhen: ${when}\nIP address: ${ip}\nDevice: ${userAgent}\n\nIf this was you, no action needed. If it wasn't, change your password immediately and review your active sessions at Profile > Sessions.`;
  const html = `<div style="font-family:sans-serif;max-width:480px;margin:auto">
<h2 style="color:#1E3A8A">New sign-in to your account</h2>
<p>Your TripMate account was just signed into from a device we haven't seen before.</p>
<table style="font-size:13px;color:#333;margin:16px 0">
<tr><td style="padding:2px 12px 2px 0;color:#666">When</td><td>${when}</td></tr>
<tr><td style="padding:2px 12px 2px 0;color:#666">IP address</td><td>${ip}</td></tr>
<tr><td style="padding:2px 12px 2px 0;color:#666">Device</td><td>${userAgent}</td></tr>
</table>
<p><strong>If this was you</strong>, no action needed.</p>
<p><strong>If it wasn't you</strong>, change your password immediately and review your active sessions from Profile &gt; Sessions.</p>
</div>`;
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"TripMate Security" <${config.SMTP_FROM_EMAIL || config.SMTP_USER || "noreply@tripmate.app"}>`,
      to: email,
      subject,
      text,
      html,
    ...
    console.error("Error sending new-login alert email:", error?.message || error);
    return false;
  }
}

// Confirm-email + set-password link — used both for a brand-new signup
// (no self-serve email/password account creation anymore, see
// auth.controller.ts's signup) and for re-confirming an old account that
// predates the emailVerified requirement. Reuses resetPassword's exact
// token mechanism and target page (/reset-password?token=) rather than
// building a second confirm-email flow — the trust proof (control the
// inbox, then choose a password) is identical either way.
export async function sendVerifyAccountEmail(email: string, token: string) {
  const url = `${config.FRONTEND_URL || "http://localhost:5000"}/reset-password?token=${token}`;
  const subject = "Confirm your TripMate account";
  const text = `Almost there — confirm this is your inbox and set a password to finish creating your TripMate account.\n\nClick the link below (valid for 1 hour):\n\n${url}\n\nIf you didn't request this, ignore this email — no account will be created.`;
  const html = `<div style="font-family:sans-serif;max-width:480px;margin:auto">
<h2 style="color:#1E3A8A">Confirm your account</h2>
<p>Click the button below to confirm this is your inbox and set a password. This link expires in <strong>1 hour</strong>.</p>
<a href="${url}" style="display:inline-block;background:#F59E0B;color:#fff;padding:12px 24px;border-radius:8px;text-decoration:none;font-weight:bold;margin:16px 0">Confirm &amp; Set Password</a>
<p style="color:#666;font-size:13px">If you didn't request this, ignore this email — no account will be created.</p>
</div>`;
  ...
  try {
    ...
    console.error("Error sending verify-account email:", error?.message || error);
    return false;
  }
}
...
export async function sendFeedbackNotificationEmail(feedback: {
  ...
  const attachmentUrls = (feedback.attachments || []).map(
    (a) => `${config.FRONTEND_URL || "http://localhost:5000"}${a}`,
  );
...
${attachmentsHtml}
  ...
  try {
    ...
    if (!transporter) throw new Error("Email transporter not initialized");
    const info = await transporter.sendMail({
      from: `"TripMate Feedback" <${config.SMTP_FROM_EMAIL || config.SMTP_USER || "noreply@tripmate.app"}>`,
      to: adminEmail,
...
export async function sendAgentTriagePlanEmail(feedback: {
  type: string;
  category: string;
  subject: string;
  description: string;
...
<div style="background:#f0f4fa;padding:12px 16px;border-radius:8px;white-space:pre-wrap;font-size:14px">${plan.replace(/\n/g, "<br/>")}</div>
  ...
  try {
    ...
    if (!transporter) throw new Error("Email transporter not initialized");
    const info = await transporter.sendMail({
      from: `"TripMate Feedback Triage" <${config.SMTP_FROM_EMAIL || config.SMTP_USER || "noreply@tripmate.app"}>`,
      to: adminEmail,
      subject,
      text,
      html,
...
): Promise<{ ok: boolean; error?: any; messageId?: string }> {
  try {
    const transporter = await transporterPromise;
