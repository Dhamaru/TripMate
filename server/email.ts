...
// Live-reported: a real feedback submission was stored fine but the site
// owner never got notified, with nothing in the logs pointing at why —
// sendFeedbackNotificationEmail/sendAgentTriagePlanEmail below silently
// return false when this is unset, same shape push.ts's VAPID check
// already warns about at boot. Warn here too so a missing ADMIN_EMAIL is
// visible in the startup logs instead of only discovered by a user asking
// "where did that feedback go".
if (!config.ADMIN_EMAIL) {
  console.warn(
    "[Email] ADMIN_EMAIL not set — feedback/triage notification emails will not be sent (feedback itself still saves normally).",
  );
}
...
${attachmentsHtml}
  ...
  if (!adminEmail) {
    // Live-reported: a real feedback submission was stored fine but the
    // site owner never got notified, with nothing in the logs to say
    // why — this silently no-op'd instead of erroring. Same silent-
    // failure shape the VAPID push keys had; log it instead this time.
    console.warn(
      "[Feedback] ADMIN_EMAIL not set — feedback was saved but no notification email was sent. Set ADMIN_EMAIL in the environment to receive these.",
    );
    return false;
  }
...
<div style="background:#f0f4fa;padding:12px 16px;border-radius:8px;white-space:pre-wrap;font-size:14px">${plan.replace(/\n/g, "<br/>")}</div>
  ...
  const adminEmail = config.ADMIN_EMAIL;
  if (!adminEmail) {
    console.warn("[Feedback] ADMIN_EMAIL not set — triage plan email was not sent.");
    return false;
  }
