...
import { guestGenerationQuota } from "../middleware/guestQuota.middleware";
...
router.post("/run", requireAuth, generationLimiter, guestGenerationQuota, runPipeline);

// Start a pipeline and stream intermediate agent results via SSE
router.post("/stream", requireAuth, generationLimiter, guestGenerationQuota, streamResults);
...
router.post("/suggestions/refresh", requireAuth, (req, res) =>
  res.json({ success: true, message: "Not implemented Yet" }),
);
router.post("/agent/map/cache/:tripId", requireAuth, (req, res) =>
  res.json({ success: true, message: "Not implemented Yet" }),
);
router.post("/agent/budget/recalculate/:tripId", requireAuth, (req, res) =>
  res.json({ success: true, message: "Not implemented Yet" }),
);
router.post("/agent/packing/regenerate/:tripId", requireAuth, (req, res) =>
  res.json({ success: true, message: "Not implemented Yet" }),
);
