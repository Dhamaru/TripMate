...
export const generateTrip = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Guest 1-trip cap — trips.controller.ts's createTrip already has
    // this exact check, but this route (the AI planner's async job
    // path) creates trips too and never had it, making the cap fully
    // bypassable by generating instead of manually creating (real,
    // live gap found by a security review this session). Check before
    // acknowledging the job, not after — a guest over the cap should
    // get a real rejection, not a fake "job started" 202 that fails
