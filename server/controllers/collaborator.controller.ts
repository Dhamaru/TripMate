...
export const addCollaborator = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Guest accounts can't invite collaborators — an anonymous, no-
    // password identity emailing a real user ("Guest Traveler added
    // you to a trip") is pure spam/trust surface with zero demo value
    // (a guest can't be collaborated with back). Checked before the
    // email lookup below so this can't double as a registered-email
    // enumeration oracle for an anonymous caller (security review
    // finding this session — the existing NotFoundError vs success
    // split on that lookup already was one). req.user.isGuest is
    // already on the JWT (issueSession signs it in) — no DB round
    // trip needed to check it.
    if (req.user?.isGuest) {
      throw new ForbiddenError("Sign in with Google to invite collaborators.");
    }
    ...
    socketService.broadcastMutation(
      tripId,
      { type: "collaborators-updated", data: updatedTrip },
      String(userId),
    );
