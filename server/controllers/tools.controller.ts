...
// TEMP diagnostic — checks whether each Atlas AI provider is actually
// reachable in production right now (key present, quota not exhausted).
// Reports status codes/error text only, never key values. Remove after use.
export const diagProviders = async (_req: Request, res: Response) => {
    const out: Record<string, any> = {};

    out.nvidia = { keyPresent: !!config.NVIDIA_API_KEY };
    if (config.NVIDIA_API_KEY) {
        try {
            ...
            out.nvidia.status = r.status;
            if (!r.ok) out.nvidia.error = (await r.text()).slice(0, 300);
        } catch (e: any) {
            out.nvidia.error = e.message;
        }
    }
    ...
    if (config.OPENAI_API_KEY) {
        try {
            ...
            if (!r.ok) out.openai.error = (await r.text()).slice(0, 300);
        } catch (e: any) {
            out.openai.error = e.message;
        }
    }

    out.gemini = { keyPresent: !!config.GEMINI_API_KEY };
    if (config.GEMINI_API_KEY) {
        try {
            ...
            if (!r.ok) out.gemini.error = (await r.text()).slice(0, 300);
        } catch (e: any) {
            out.gemini.error = e.message;
        }
    }
