...
`.trim();
    ...
    try {
      const gptRes: any = await withRetries(
        () =>
          this.openai.chat.completions.create({
            model: "gemini-3.6-flash",
            response_format: { type: "json_object" },
            // A system-only messages array translates to an empty `contents`
            // on Gemini's OpenAI-compat endpoint and gets rejected outright
            // (400 INVALID_ARGUMENT) -- OpenAI tolerated this, Gemini
            // requires at least one non-system turn. There's no separate
            // system/user split here anyway, so just send it as user.
      ...
      console.error(
        `[CriticAgent] LLM critique failed, falling back to quantitative check:`,
        e?.message,
      );
      return {
        valid: results.length === 0,
